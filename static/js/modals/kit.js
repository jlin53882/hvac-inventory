...
async function submitKit() {
  ...
  try {
    const res = await fetch('/api/kits', {
      ...
      body: JSON.stringify({ name: name, brand: document.getElementById('k-brand').value.trim(), code: document.getElementById('k-code').value.trim(), items: items, note: document.getElementById('k-note').value.trim() })
    });
