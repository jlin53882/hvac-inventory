...
function renderKitActionButtons(k, isViewer, isM, status) {
  if (isViewer) return '';
  if (isM) return `<div class="kit-mobile-actions"><button class="kit-action is-prepare" onclick="openPrepareModal(${k.item_id}, event)">📤 待領出</button><button class="kit-action is-out" onclick="openOutModal(${k.item_id}, event)">🚚 已領出</button>${hasPerm('stock-mgmt') ? `<button class="kit-action" onclick="openTransferModal(${k.item_id})">🔄 調撥</button>` : ''}<button class="kit-more" type="button" onclick="openKitSheet(${k.id})" aria-label="整組操作">⋯</button></div>`;
