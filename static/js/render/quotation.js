...
function quoteCanManage() {
  return typeof hasPerm === 'function' && hasPerm('item-mgmt');
}
  ...
  el.innerHTML = `
    <div class="quote-wrap">
      ...
      <div class="dsr-page-header quote-page-header">
        ...
        <div class="dsr-page-actions">${canManage ? `<button class="dsr-btn dsr-btn--ghost" onclick="quoteReset()">清除表單</button><button class="dsr-btn dsr-btn--primary" onclick="quoteSave()">💾 ${esc(saveLabel)}</button>` : ''}</div>
      </div>
...
async function quoteLoadHistory() { var list = document.getElementById('quote-history-list'); if (!list) return; var q = document.getElementById('quote-history-q')?.value.trim() || ''; try { quotationHistory = []; var page = 1; var total = Infinity; while (quotationHistory.length < total && page <= 100) { var res = await fetch('/api/quotations?q=' + encodeURIComponent(q) + '&page=' + page + '&page_size=100'); var data = await res.json(); if (!res.ok) throw new Error(data.detail || '歷史報價單載入失敗'); quotationHistory = quotationHistory.concat(data.items || []); total = Number(data.total || 0); if (!(data.items || []).length) break; page += 1; } list.innerHTML = quotationHistory.map(function(item) { return `<div class="quote-history-row"><div><strong>${esc(item.quote_number)}</strong><span>${esc(item.customer_name)}</span><small>${esc(item.quote_date)} · ${esc(item.tax_type === 'included' ? '含稅' : '未稅')}</small></div><strong class="quote-history-total">$${esc(String(quoteMoney(item.total)))}</strong><div class="quote-history-actions">${quoteCanManage() ? `<button class="dsr-action-btn" onclick="quoteEdit(${esc(String(item.id))})">編輯</button>` : ''}<button class="dsr-action-btn" onclick="quoteDownload(${esc(String(item.id))}, 'xlsx')">Excel</button><button class="dsr-action-btn" onclick="quoteDownload(${esc(String(item.id))}, 'pdf')">PDF</button>${quoteCanManage() ? `<button class="dsr-action-btn dsr-action-btn--danger" onclick="quoteDelete(${esc(String(item.id))})">刪除</button>` : ''}</div></div>`; }).join(''); var empty = document.getElementById('quote-history-empty'); if (empty) empty.style.display = quotationHistory.length ? 'none' : 'block'; } catch (e) { list.innerHTML = `<div class="dsr-empty">⚠️ ${esc(e.message)}</div>`; } }
