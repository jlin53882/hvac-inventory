...
#
# 用法: ./scripts/run-tests.sh [core|frontend|storage|auth|database|rbac|regression|petty_cash|work_progress|all]
#   組別    內容                                      觸發條件（改到就跑這組）
...
case "${1:-all}" in
  ...
  all)        FILES="tests/" ;;
  *) echo "用法: $0 [core|frontend|storage|auth|database|rbac|regression|petty_cash|work_progress|all]"; exit 1 ;;
esac
